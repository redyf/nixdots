...
let
  ...
  wrapFirefoxCompat = { version, pkg }:
    let
      ...
      nameArg =
        if wrapperHasArg "applicationName"
      ...
      extraArgs =
        if wrapperHasArg "wmClass"
        then { inherit (version) wmClass; }
        else { };

      allArgs = requiredArgs // extraArgs;
    in
    wrapper.override allArgs;
  ...
  firefoxVersion = version:
    ...
    in
    wrapFirefoxCompat { inherit version pkg; };
...

{
  ...
  # reproducible builds.
  latest = (super.latest or { }) // (builtins.mapAttrs (n: firefoxVersion) firefoxVariants);
